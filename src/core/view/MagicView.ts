...
import { ScalePlugin } from './ScalePlugin.ts';
      ...
      switch (commit.type) {
        case 'appendChild':
          if (commit.node.parent) {
            const findNode = this.findOne(commit.node.parent.id);
          ...
          break;
        case 'deleteChild':
          this.findOne(commit.node.id)?.destroy();
          break;
        case 'updateAttrs':
          applyNodeProps(this.findOne(commit.node.id), commit.node.props, commit.old?.props || {});
          break;
        case 'moveChild':
          const parent = this.findOne(commit.node.parent!.id) as Group;
